...
public class GhostSprites : MonoBehaviour
{
    ...
    public void ShowTrail(Vector3 position)
    {
        for (int i = 0; i < ghostList.Count; i++)
        {
            ghostList[i].SetActive(true);
            ghostList[i].transform.position = position;
        }
    }
	...
	void Update ()
	{
        ...
        if (ghostMaterial.Count == 0)
        {
            ...
            //sprite.flipX = GetComponent<SpriteRenderer>().flipX;
    ...
    public void DeactivateRenderer()
    {
        for (int i = 0; i < ghostList.Count; i++)
        {
            ghostList[i].GetComponent<SpriteRenderer>().enabled = false;
        }
    }
    public void ActivateRenderer()
    {
        for (int i = 0; i < ghostList.Count; i++)
        {
            ghostList[i].GetComponent<SpriteRenderer>().enabled = true;
        }
    }
	...
	private void Populate(Vector3 position, bool allowPositionOverride)
	{
		...
		g.AddComponent<SpriteRenderer>().flipX = GetComponent<SpriteRenderer>().flipX;
